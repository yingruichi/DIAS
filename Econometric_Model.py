...
class EconometricModelApp:
    ...
    def load_data(self):
        ...
        excel_file_path = os.path.join(excel_dir, 'Econometric Model.xlsx')
        ...
        self.first_column, self.second_column, self.third_column, self.fourth_column = zip(*sorted_data) if sorted_data else (
        [], [], [], [])
    ...
    def create_ui(self):
        ...
        # 创建按钮
        fixed_button_width = 20  # 设置固定的按钮宽度
        for index, button_text in enumerate(self.first_column):
            if self.current_language == 'zh':
                display_text = button_text
            else:
                display_text = self.second_column[index]
            button = ttk.Button(inner_frame, text=display_text, bootstyle=PRIMARY, width=fixed_button_width)
            row = index // 4
            col = index % 4
        ...
        self.switch_language_label = ttk.Label(self.root, text=self.LANGUAGES[self.current_language]['switch_language'],
                                               foreground='gray',
                                               cursor='hand2')
