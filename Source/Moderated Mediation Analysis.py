...
def moderated_mediation_analysis(data, ind_var, med_var, dep_var, mod_var):
    # 第一步：自变量对因变量的总效应
    X1 = data[ind_var]
...
def analyze_file():
    ...
    try:
        ...
        # 进行调节中介作用分析
        total_effect, p_value_total, effect_ind_med, p_value_ind_med, effect_mod_ind_med, p_value_mod_ind_med, effect_med_dep, p_value_med_dep, effect_mod_med_dep, p_value_mod_med_dep, mediation_effect, sample_size = moderated_mediation_analysis(
        ...
        explanation_df = explanation_df.reindex(
            columns=["自变量对因变量的总效应", "自变量对中介变量的效应", "调节变量对自变量 - 中介变量关系的调节效应",
                     "中介变量对因变量的效应（控制自变量）", "调节变量对中介变量 - 因变量关系的调节效应", "中介效应", "样本量"])
        ...
        if save_path:
            ...
            labels = ["自变量对因变量总效应", "自变量对中介变量效应", "调节变量对自变量 - 中介变量关系调节效应", "中介变量对因变量效应",
                      "调节变量对中介变量 - 因变量关系调节效应", "中介效应"] if current_language == 'zh' else [
            ...
            ax.set_title('调节中介作用分析结果' if current_language == 'zh' else 'Moderated Mediation Analysis Results')
