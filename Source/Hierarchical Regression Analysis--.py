...
def analyze_file():
    ...
    try:
        ...
        for i in range(1, X.shape[1] + 1):
            ...
            results.append([f"Model {i}"] + list(coefficients) + [intercept, mse, r2, adjusted_r2, f_value] + list(t_values) + list(p_values))

        # 准备数据
        columns_stats = ["Coefficients", "t-value", "p-value", "R-squared (R²)", "Adjusted R-squared", "F-value"]
        explanations = LANGUAGES[current_language]['explanation']
        # 修正 headers 列表
        headers = ["Model"] + [f"Coefficient_{i+1}" for i in range(X.shape[1])] + ["Intercept", "MSE", "R-squared (R²)", "Adjusted R-squared", "F-value"] + [f"t-value_{i+1}" for i in range(X.shape[1])] + [f"p-value_{i+1}" for i in range(X.shape[1])]
