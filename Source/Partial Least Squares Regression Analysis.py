...
def analyze_file():
    ...
    try:
        ...
        # 将 n_components 调整为 1
        pls_scaled = PLSRegression(n_components=1)
        pls_scaled.fit(X_scaled, y)
        standardized_coefficients = pls_scaled.coef_.flatten()
        ...
        data = [["Partial Least Squares Regression"] + list(coefficients) + [intercept, mse, r2, adjusted_r2, f_value] + list(standardized_coefficients) + list(t_values) + list(p_values)]
