...
def analyze_file():
    ...
    try:
        ...
        # 进行分层聚类分析
        labels, linkage_matrix = hierarchical_clustering(data)
        ...
        if save_path:
            ...
            plt.title('聚类树状图' if current_language == 'zh' else 'Hierarchical Clustering Dendrogram')
            plt.xlabel('样本编号' if current_language == 'zh' else 'Sample Index')
            plt.ylabel('距离' if current_language == 'zh' else 'Distance')
            # 保存图片
