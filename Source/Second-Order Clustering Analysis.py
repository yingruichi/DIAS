...
def second_order_clustering(data, n_clusters=3):
    """
    进行二阶聚类分析
    :param data: 输入数据
...
def analyze_file():
    ...
    try:
        ...
        # 进行二阶聚类分析
        labels, model = second_order_clustering(data)
        ...
        if save_path:
            ...
            plot_dendrogram(model, labels=range(data.shape[0]))
            plt.title('聚类树状图' if current_language == 'zh' else 'Second-Order Clustering Dendrogram')
