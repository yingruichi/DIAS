...
def analyze_file():
    ...
    try:
        ...
        # 进行折半信度分析
        split_half = split_half_reliability(numerical_df)
        ...
        explanation_df = explanation_df.reindex(columns=["折半信度系数", "样本量", "均值"])
        explanation_df.insert(0, "统计量_解释说明", "解释说明" if current_language == 'zh' else "Explanation")

        # 添加分析结果解读
        interpretation_df = pd.DataFrame([interpretations])
        interpretation_df = interpretation_df.reindex(columns=["折半信度系数", "样本量", "均值"])
