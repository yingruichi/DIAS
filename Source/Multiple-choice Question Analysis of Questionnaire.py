...
LANGUAGES = {
    ...
    'en': {
        'title': "Multiple-choice Question Analysis of Questionnaire",
        ...
        'explanation': {
            "选项选择次数": "The total number of times each option was selected.",
            "样本量": "The total number of people participating in the questionnaire."
        },
        'interpretation': {
            "选项选择次数": "The more times an option is selected, the more popular it is in the questionnaire.",
...
def analyze_file():
    ...
    try:
        ...
        # 进行多选题分析
        choice_counts, sample_size = multiple_choice_analysis(df)

        # 整理数据
        data = [
            ["选项选择次数", pd.Series(choice_counts).to_dict(), ""],
            ["样本量", sample_size, ""]
        ]
        headers = ["统计量", "统计量值", "p值"]
        ...
        explanation_df = explanation_df.reindex(columns=["选项选择次数", "样本量"])
        explanation_df.insert(0, "统计量_解释说明", "解释说明" if current_language == 'zh' else "Explanation")

        # 添加分析结果解读
        interpretation_df = pd.DataFrame([interpretations])
        interpretation_df = interpretation_df.reindex(columns=["选项选择次数", "样本量"])
        ...
        if save_path:
            ...
            ax.set_title('选项选择次数柱状图' if current_language == 'zh' else 'Bar Chart of Option Selection Counts')
            ax.set_xlabel('选项' if current_language == 'zh' else 'Options')
            ax.set_ylabel('选择次数' if current_language == 'zh' else 'Selection Counts')
            # 保存图片
