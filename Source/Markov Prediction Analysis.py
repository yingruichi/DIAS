...
def markov_prediction(data, num_periods=5):
    """
    进行马尔可夫预测
    :param data: 状态转移矩阵数据
...
def analyze_file():
    ...
    try:
        ...
        # 进行马尔可夫预测
        transition_matrix, prediction_results = markov_prediction(data)
        ...
        if save_path:
            ...
            ax.set_title('马尔可夫预测结果折线图' if current_language == 'zh' else 'Line Chart of Markov Prediction Results')
            ax.set_xlabel('时期' if current_language == 'zh' else 'Periods')
            ax.set_ylabel('状态' if current_language == 'zh' else 'States')
            # 保存图片
