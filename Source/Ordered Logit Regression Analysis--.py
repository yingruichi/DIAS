...
def analyze_file():
    ...
    try:
        ...
        logit_model = sm.OrderedModel(y, X_with_const, distr='logit')
        ...
        # 准备数据
        columns_stats = ["Coefficients", "z-value", "p-value", "Accuracy"]
        explanations = LANGUAGES[current_language]['explanation']
        data = [["Ordered Logit Regression"] + list(coefficients) + [accuracy] + list(z_values) + list(p_values)]

        headers = ["Model"] + [f"Coefficient_{i+1}" for i in range(len(coefficients))] + columns_stats[3:] + [f"z-value_{i+1}" for i in range(len(z_values))] + [f"p-value_{i+1}" for i in range(len(p_values))]
