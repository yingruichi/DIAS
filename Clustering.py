...
class ClusteringApp:
    ...
    def load_data(self):
        # 读取 Excel 文件，使用相对路径
        excel_file_path = 'Excel/Clustering.xlsx'
        try:
            # 获取当前脚本所在的目录
            current_dir = os.path.dirname(os.path.abspath(__file__))
            # 构建 Excel 文件的相对路径
            excel_file_path = os.path.join(current_dir, 'Excel', 'Clustering.xlsx')
    ...
    def create_ui(self):
        ...
        self.switch_language_label = ttk.Label(self.root, text=self.LANGUAGES[self.current_language]['switch_language'], foreground='gray',
                                      cursor='hand2')
