...
class DataDescriptionApp:
    def __init__(self, root=None):
        ...
        self.LANGUAGES = {
            ...
            'en': {
                'title': "(DIAS) Data Description and Validation",
        ...
        self.load_data()
        # 计算按钮的统一宽度
        self.button_width = self.calculate_max_button_width()
        self.create_ui()
    ...
    def load_data(self):
        ...
        # 构建 Excel 文件的相对路径
        excel_file_path = os.path.join(current_dir, 'Excel', 'Data Description and Validation.xlsx')
    ...
    def calculate_max_button_width(self):
        """计算所有按钮文本的最大宽度，以英文字符数为单位"""
        # 确保数据已加载
        if not hasattr(self, 'first_column') or not hasattr(self, 'second_column'):
            return 20  # 默认宽度

        # 计算所有中文和英文文本的最大长度
        max_chinese_length = max(len(str(text)) for text in self.first_column) if self.first_column else 0
        max_english_length = max(len(str(text)) for text in self.second_column) if self.second_column else 0

        # 考虑到中文字符通常比英文字符宽，进行适当调整
        # 这里假设一个中文字符相当于2个英文字符的宽度
        adjusted_chinese_length = max_chinese_length * 2

        # 返回最大宽度，增加一些余量
        return max(adjusted_chinese_length, max_english_length) + 2
    ...
    def switch_language(self):
        ...
        for index, button in enumerate(self.button_list):
            ...
            button.config(text=display_text, width=self.button_width)
    ...
    def create_ui(self):
        ...
        for index, button_text in enumerate(self.first_column):
            ...
            button = ttk.Button(
                ...
                text=display_text,
                bootstyle=PRIMARY,
                width=self.button_width  # 设置统一宽度
            )
            row = index // 3
            col = index % 3
